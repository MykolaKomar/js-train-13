...
function searchParamsURL(url) {
  ...
  for (const [key, velue] of URLSearchParams) {
    params.set(key, velue);
  }
...
function modifyURLParameters(url, params) {
  ...
  for (const key in params) {
    ...
    const velue = params[key];
    // Якщо параметр вже існує, метод set замінює його новим значенням.
    if (urlObj.searchParams.has(velue)) {
      urlObj.searchParams.set(key, velue);
    } else {
...
function checkURLParameters(url, params) {
  ...
  for (const elem of params) {
    newObj[elem] = urlObj.searchParams.has(elem);
  }
...
function processUrl(url, options) {
  ...
  if (options.searchParams) {
    ...
    for (const key in options.searchParams) {
      ...
      const velue = options.searchParams[key];
      urlObj.searchParams.append(key, velue);
    }
  }
